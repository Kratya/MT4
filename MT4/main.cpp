			...
			while (1)
			{
				...
				if (ch >= '0' && ch <= '9')
				{
					do
					{
						...
						if (ch >= 'a' && ch <= 'z' || ch >= 'A' && ch <= 'Z')
						{
							errorFile << "Lexical error: unexpected const " << str + ch << endl;
							return false;
						}
					} while (ch >= '0' && ch <= '9');
				...
				if (ch == '+' || ch == '-' || ch == '*' || ch == '!' || ch == '<' || ch == '>')
				{
					...
					else { errorFile << "Lexical error: unexpected symbol " << ch << endl; return false; }
					str.clear();
				}
				...
				if (ch == '(' || ch == ')' || ch == '{' || ch == '}' || ch == ':' || ch == ';' || ch == ',' || ch == '.')
				{
					...
					else { errorFile << "Lexical error: unexpected symbol" << str << endl; return false; }
					str.clear();
					if (code.peek() == EOF) { return true; }
					code.get(ch);
				}
				...
				if (ch == '/')
				{
					if (code.peek() == EOF) { errorFile << "Lexical error: unexpected symbol";	return false; }
					code.get(ch);
					if (ch == '/')
						do
						{
					...
					else if (ch == '*')
					{
						...
						do
						{
							if (code.peek() == EOF) { errorFile << "Lexical error: unclosed comment";	return false; }
							code.get(ch);
							if (ch == '*')
							{
								if (code.peek() == EOF) { errorFile << "Lexical error: unclosed comment";	return false; }
					...
					else { errorFile << "Lexical error: open comment error\n"; return false; }
					unexpected = false;
					if (code.peek() == EOF) { return true; }
					code.get(ch);
				}
				...
				if (unexpected)
				{
					errorFile << "Lexical error: unexpected symbol " << ch << endl; return false;
				}
			}
		}
		tokenFile.close();
